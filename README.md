...
AI coding agents are stateless: every session starts with amnesia. The agent that broke your build editing `vercel.json` on Monday will happily try the exact same edit on Thursday. `projectmemory` gives each repo a persistent memory of **failed attempts, fragile files, decisions, and deployment rules** — and *force-feeds* the relevant warning to the agent at the moment it's about to repeat history.

Not a notebook the agent may choose to read. A guardrail it can't skip.

## How it works

1. **Memories live in your repo** — a `.projectmemory/` folder with a local SQLite index and human-readable, PR-reviewable markdown views. Local-first: nothing leaves your machine.
...
```bash
cd your-repo
npx -y projectmemory init       # creates .projectmemory/ + installs Claude Code hooks

# record your first guardrail
npx -y projectmemory remember failed_attempt \
...
```bash
claude mcp add projectmemory -- npx -y projectmemory mcp   # Claude Code MCP
npx -y projectmemory install-mcp --write cursor            # Cursor
npx -y projectmemory install-mcp                           # print generic config
```
...
|---|---|
| `pm init` | Set up `.projectmemory/`, gitignore, and hooks |
...
- **Human-readable, PR-able.** Every memory renders to markdown under `.projectmemory/` that your team reviews like any other change.
